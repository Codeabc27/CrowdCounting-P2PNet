...
def _vgg(arch, cfg, batch_norm, pretrained, progress, sync=False, **kwargs):
    ...
    if pretrained:
        # Prefer local checkpoint if available (legacy path), otherwise download from official URL
        if arch in model_paths and os.path.exists(model_paths[arch]):
            state_dict = torch.load(model_paths[arch], map_location="cpu")
        else:
            # fallback to official torchvision URL
            url = model_urls.get(arch)
            if url is None:
                raise ValueError(f"No pretrained weights URL for arch '{arch}'")
